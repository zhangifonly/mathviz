...
export function drawRomanSurface(canvas: HTMLCanvasElement, opts: DrawOptions): void {
  ...
  const grid = normalizeGrid(
    sampleSurface(
      (u, v) => romanSurface(u, v, a),
      U_RANGE, V_RANGE, 80, 80,
    ),
  )
...
function drawLabel(ctx: CanvasRenderingContext2D, o: DrawOptions, W: number): void {
  ...
  ctx.fillText('罗马曲面', 18, 30)
  ctx.font = '13px sans-serif'
  ctx.fillStyle = 'rgba(255,255,255,0.62)'
  ctx.fillText('尺度 a = ' + o.a.toFixed(2), 18, 52)
  ctx.textAlign = 'right'
  ctx.fillText('斯坦纳的四次曲面', W - 18, 30)
  ctx.restore()
}
