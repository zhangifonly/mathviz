...
export function drawDiniSurface(canvas: HTMLCanvasElement, opts: DrawOptions): void {
  const ctx = canvas.getContext('2d')
  if (!ctx) return
  const { a, b, yaw = 0.6, ramp = 'viridis' } = opts
...
function drawLabel(ctx: CanvasRenderingContext2D, o: DrawOptions, W: number): void {
  ...
  ctx.fillText('迪尼曲面', 18, 30)
  ctx.font = '13px sans-serif'
  ctx.fillStyle = 'rgba(255,255,255,0.62)'
  ctx.fillText('粗细 a = ' + o.a.toFixed(2) + '   ' + '螺距系数 b = ' + o.b.toFixed(2), 18, 52)
  ctx.textAlign = 'right'
  ctx.fillText('螺旋的常负曲率曲面', W - 18, 30)
  ctx.restore()
}
