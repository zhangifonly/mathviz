...
export function drawCrossCap(canvas: HTMLCanvasElement, opts: DrawOptions): void {
  const ctx = canvas.getContext('2d')
  if (!ctx) return
  const { height, yaw = 0.6, ramp = 'coolwarm' } = opts
...
function drawLabel(ctx: CanvasRenderingContext2D, o: DrawOptions, W: number): void {
  ...
  ctx.fillText('交叉帽', 18, 30)
  ctx.font = '13px sans-serif'
  ctx.fillStyle = 'rgba(255,255,255,0.62)'
  ctx.fillText('纵向拉伸 = ' + o.height.toFixed(2), 18, 52)
  ctx.textAlign = 'right'
  ctx.fillText('射影平面最简单的浸入', W - 18, 30)
  ctx.restore()
}
