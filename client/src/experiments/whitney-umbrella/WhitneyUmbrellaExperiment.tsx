...
import { whitneyUmbrellaNarration } from '../../narrations/scripts/whitney-umbrella'
import { usePresenterHistory } from '../../hooks/usePresenterHistory'
import Surface3DPlot from '../../lib/Surface3DPlot'
import { whitneyUmbrella, U_RANGE, V_RANGE, PRESETS } from './whitneyUmbrella'

// 知识卡条目。抽成常量而不是内联 JSX, 免得组件超过 100 行
const FACTS = [
  ['(u,v) ↦ (uv, u, v²)', '，三个单项式就给出一类稳定奇点的标准模型。'],
  ['隐式方程 x² = y²z', '，参数化的点代入残差精确为零。'],
  ['雅可比在原点退化', '：秩从 2 掉到 1，映射不再是浸入，这就是伞点。'],
  ['两片夹角趋于零', '：沿自交线接近伞点时两片几乎贴合，区别于横截自交。'],
]

export default function WhitneyUmbrellaExperiment() {
  ...
  useEffect(() => {
    if (narration) narration.loadScript(whitneyUmbrellaNarration)
  }, [narration])
    ...
    <>
      ...
      <div className="space-y-6">
        <header className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">惠特尼伞</h1>
            <p className="text-gray-600">光滑映射的稳定奇点</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-white rounded-lg shadow-sm border border-gray-200 p-4">
            ...
            <Surface3DPlot
              fn={(u, v) => whitneyUmbrella(u, v, scale)}
              uRange={U_RANGE}
              vRange={V_RANGE}
              colorscale="RdBu"
              deps={[scale]}
