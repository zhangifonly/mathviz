...
import { developableSurfaceNarration } from '../../narrations/scripts/developable-surface'
import { usePresenterHistory } from '../../hooks/usePresenterHistory'
import { drawDevelopable } from './draw'
import { DEV_INFO, infoOf, type DevKind } from './developableSurface'

// 知识卡条目。抽成常量而不是内联 JSX, 免得组件超过 100 行
const FACTS: Array<[string, string]> = [
  ['只有三类', '：柱面(方向恒定)、锥面(共顶点)、切线面(曲线的切线族)。'],
  ['两条等价判据', '：det[γ′,d,d′] ≡ 0，或高斯曲率 K ≡ 0。'],
  ['直纹 ≠ 可展', '：螺旋面是直纹面，但 det=0.33、K=4.87，摊不平。'],
  ['工程依赖它', '：船体外板要用平钢板压制，服装要先在平面裁片。'],
]

const W = 640
const H = 480

export default function DevelopableSurfaceExperiment() {
  const [kind, setKind] = useState<DevKind>('cylinder')
  ...
  useEffect(() => {
    if (narration) narration.loadScript(developableSurfaceNarration)
  }, [narration])
  ...
  useEffect(() => {
    ...
    const loop = (ts: number) => {
      ...
      drawDevelopable(canvas, { kind, showRulings: true, showCriteria: true, surfaceAlpha: 0.6, yaw: 0.6 + el * 0.26 })
    ...
    <>{showPresenter && <NarrationPresenter onExit={handleExit} />}
      <div className="space-y-6">
        <header className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-800">可展曲面</h1>
            <p className="text-gray-600">能摊平的三类曲面</p>
          </div>
