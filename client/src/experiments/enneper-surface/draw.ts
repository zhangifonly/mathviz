...
export function drawEnneperSurface(canvas: HTMLCanvasElement, opts: DrawOptions): void {
  const ctx = canvas.getContext('2d')
  if (!ctx) return
  const { scale, yaw = 0.6, ramp = 'plasma' } = opts
  ...
  const grid = normalizeGrid(
    sampleSurface(
      (u, v) => enneper(u, v, scale),
      U_RANGE, V_RANGE, 60, 60,
    ),
  )
...
function drawLabel(ctx: CanvasRenderingContext2D, o: DrawOptions, W: number): void {
  ...
  ctx.fillText('恩内佩尔曲面', 18, 30)
  ctx.font = '13px sans-serif'
  ctx.fillStyle = 'rgba(255,255,255,0.62)'
  ctx.fillText('缩放 = ' + o.scale.toFixed(2), 18, 52)
  ctx.textAlign = 'right'
  ctx.fillText('会自交的极小曲面', W - 18, 30)
  ctx.restore()
}
