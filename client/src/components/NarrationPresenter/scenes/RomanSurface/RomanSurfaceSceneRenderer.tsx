...
import { drawRomanSurface } from '../../../../experiments/roman-surface/draw'
...
function TitleScene({ sceneId }: { sceneId: string }) {
  const titles: Record<string, { title: string; subtitle: string }> = {
    'intro-1': { title: '罗马曲面', subtitle: '斯坦纳的四次曲面' },
    'sum-3': { title: '感谢观看', subtitle: '探索数学之美' },
  }
  const { title, subtitle } = titles[sceneId] || { title: '罗马曲面', subtitle: '' }
  return (
...
function SummaryScene({ sceneId }: { sceneId: string }) {
  const items: Record<string, string[]> = {
    'sum-1': ['斯坦纳 1844 年发现', '四次代数曲面', '射影平面的浸入'],
    'sum-2': ['三条沿坐标轴的自交线', '六个分支点', '原点处一个三重点'],
  }
...
export default function RomanSurfaceSceneRenderer({ scene }: SceneRendererProps) {
