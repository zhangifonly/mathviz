...
import { drawWhitneyUmbrella } from '../../../../experiments/whitney-umbrella/draw'
...
function TitleScene({ sceneId }: { sceneId: string }) {
  const titles: Record<string, { title: string; subtitle: string }> = {
    'intro-1': { title: '惠特尼伞', subtitle: '光滑映射的稳定奇点' },
    'sum-3': { title: '感谢观看', subtitle: '探索数学之美' },
  }
  const { title, subtitle } = titles[sceneId] || { title: '惠特尼伞', subtitle: '' }
  return (
...
function SummaryScene({ sceneId }: { sceneId: string }) {
  const items: Record<string, string[]> = {
    'sum-1': ['平面到空间的光滑映射', '一般位置下只有两类稳定奇点', '惠特尼 1944 年的分类'],
    'sum-2': ['原点处雅可比退化', '不再是浸入', '两片曲面在此贴合'],
  }
...
export default function WhitneyUmbrellaSceneRenderer({ scene }: SceneRendererProps) {
